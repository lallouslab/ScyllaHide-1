...
bool startInjection(DWORD targetPid, const WCHAR * dllPath)
{
    ...
    HANDLE hProcess = OpenProcess(
            PROCESS_SUSPEND_RESUME | PROCESS_CREATE_THREAD | PROCESS_VM_OPERATION | PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_QUERY_INFORMATION | PROCESS_SET_INFORMATION,
            0,
            targetPid);
...
int wmain(int argc, wchar_t* argv[])
{
    ...
    if (targetPid != 0 && dllPath != nullptr)
    {
    ...
    {
        wchar_t *processName = wcsrchr(argv[0], '\\');
        ...
        wprintf(L"Usage: %s pid:<process id> <dll path> [nowait]", processName);
    }
